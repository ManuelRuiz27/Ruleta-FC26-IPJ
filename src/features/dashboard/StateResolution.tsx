...
export default function StateResolution() {
  ...
  const getDuplicateTeamsState = useTournamentStore(state => state.getDuplicateTeamsState);
  const getAvailableTeamsForState = useTournamentStore(state => state.getAvailableTeamsForState);
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[var(--color-muted)] mt-1">Nivel: Estatal</p>
        </div>
        <button 
      ...
      {duplicates.length === 0 ? (
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-8 text-center">
          ...
          <p className="text-[var(--color-muted)] mb-6">Todas las selecciones estatales son únicas.</p>
          <button 
            onClick={() => navigate(`/estatal/dashboard`)}
              ...
              <div key={dup.team_id} className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-6">
                <div className="flex items-center justify-between mb-6 border-b border-[var(--color-border)] pb-4">
                  <div>
                    ...
                    <p className="text-sm text-[var(--color-muted)] mt-1">{dup.occurrences.length} ocurrencias detectadas en distintas regiones</p>
                      ...
                      <div key={occ.qualified_player_id} className={`p-4 rounded-lg border ${isKeeper ? 'border-green-500/30 bg-green-500/5' : isAffected ? 'border-orange-500/30 bg-orange-500/5' : 'border-[var(--color-border)] bg-black/20'}`}>
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            ...
                            <div className="text-sm text-[var(--color-muted)]">Región: {occ.region_name}</div>
                          </div>
                          <span className={`text-xs px-2 py-0.5 rounded uppercase font-bold ${occ.rank === 'champion' ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-500/20 text-slate-300'}`}>
                            {occ.rank === 'champion' ? 'Campeón Regional' : 'Subcampeón Regional'}
                          </span>
                        </div>
